...
class SpeakerEncoder:
    ...
    def verify(
        ...
        """
        Verify whether the speaker in `wav` matches the enrolled `voiceprint`.

        Args:
            wav:        Raw audio samples (float32).
            sr:         Sample rate of wav.
            voiceprint: Stored 256-dim embedding from enrollment.
            threshold:  Cosine similarity threshold (e.g. 0.72).

        Returns:
            True  if similarity >= threshold  (accepted)
            True  if encoder failed to load   (fail-open)
            False if similarity < threshold   (rejected)

        Never raises.
        """
        if not self._loaded and not self._ensure_loaded():
            # Encoder unavailable — fail open so Vani keeps working
            log.debug("speaker_encoder: verify() — encoder not loaded, failing open")
            return True

        try:
            embedding = self.embed(wav, sr)
            if embedding is None:
                log.debug("speaker_encoder: verify() — embed returned None, failing open")
                return True

            sim = self.similarity(embedding, voiceprint)
            accepted = sim >= threshold

            log.debug(
                "speaker_encoder: verify() similarity=%.4f threshold=%.4f accepted=%s",
                sim, threshold, accepted,
            )
            return accepted
